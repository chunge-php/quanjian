...
if (!root) { console.error('用法: node flatten-node-modules.cjs <包目录>'); process.exit(1) }
const nm = path.join(root, 'node_modules')
const store = path.join(nm, '.pnpm')
if (!fs.existsSync(store)) { console.log('   node_modules 已是平铺布局，不用处理'); process.exit(0) }
...
const version = (p) => { try { return JSON.parse(fs.readFileSync(path.join(p, 'package.json'), 'utf8')).version } catch { return '?' } }
...
for (const id of fs.readdirSync(store)) {
  ...
  for (const n of fs.readdirSync(base)) {
    if (n.startsWith('@')) { for (const s of fs.readdirSync(path.join(base, n))) names.push(`${n}/${s}`) } else names.push(n)
  }
  for (const name of names) {
    const src = path.join(base, name)
    if (isLink(src) || !isDir(src)) continue
    const prev = pkgs.get(name)
    if (prev && version(prev) !== version(src)) console.log(`   [注意] ${name} 有两个版本 ${version(prev)} / ${version(src)}，平铺只留前者`)
    if (!prev) pkgs.set(name, src)
  }
}
// 先把顶层软链拆掉，再把真实目录拷到顶层；全部拷完再删 .pnpm（拷贝时 dereference，包内若还有软链一并展开）
for (const name of pkgs.keys()) { const dest = path.join(nm, name); if (isLink(dest)) fs.rmSync(dest, { force: true }) }
let n = 0
...
for (const f of ['.modules.yaml', '.pnpm-workspace-state.json']) fs.rmSync(path.join(nm, f), { force: true })
...
try {
  ...
  console.error(`   [失败] 平铺后 next / styled-jsx 仍解析不到：${e.message}`); process.exit(1)
}
